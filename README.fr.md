# API Auto-Modifiable (Self Modifying API)

AVERTISSEMENT : VEUILLEZ NE PAS UTILISER CECI EN PRODUCTION - PREUVE DE CONCEPT UNIQUEMENT

## Qu'est-ce que c'est

C'est une tentative de créer une API qui peut accepter des requêtes POST qui entraînent des modifications de sa propre base de code et donc de sa propre interface API.
...
1. Accepte du code Python sous forme de chaîne de caractères
   1. Nous attendons un format particulier - voir `healthcheck.py` comme référence
...
C'est un projet FastAPI, donc installez les dépendances depuis `requirements.txt` et exécutez `./bin/dev` pour démarrer le serveur de développement en local.
...
Les nouvelles dépendances ne sont actuellement pas gérées (par exemple, si un code Python pour un nouvel endpoint utilise numpy, nous n'essayons pas d'installer les dépendances manquantes).