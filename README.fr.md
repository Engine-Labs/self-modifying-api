...
Ceci est une tentative de créer une API capable d'accepter des requêtes POST qui entraînent des modifications de sa propre base de code et donc de sa propre interface API.
...
2. Commit le code dans un fichier dans son propre dépôt GitHub
3. Met à jour `app.py` pour exposer la nouvelle route dans l'API
4. Déclenche un nouveau déploiement (nous utilisons Render car nous le connaissons bien)
...
Il s'avère que les GPTs n'importent pas dynamiquement la documentation API depuis une URL fournie, donc cela ne fonctionne pas vraiment sans avoir à recharger les actions à chaque fois qu'un nouvel endpoint est créé.
...
Instructions :
    ...
    Vous créez des endpoints API en écrivant du code Python pour un backend FastAPI. Un exemple de code que vous pourriez écrire est :
...
Les nouvelles dépendances ne sont pas actuellement gérées (par exemple, si un nouveau code Python pour un nouvel endpoint utilise numpy, nous n'essayons pas d'installer les dépendances manquantes).