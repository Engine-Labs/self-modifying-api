...
Il s'agit d'une tentative de créer une API capable d'accepter des requêtes POST qui entraînent des modifications de sa propre base de code et donc de sa propre interface API.
...
2. Enregistre le code dans un fichier dans son propre dépôt GitHub
...
Il s'avère que les GPTs n'importent pas dynamiquement la documentation API à partir d'une URL fournie, donc cela ne fonctionne pas vraiment sans devoir recharger les actions à chaque fois qu'un nouvel endpoint est créé.
...
Les nouvelles dépendances ne sont actuellement pas gérées (par exemple, si un nouveau code Python pour un nouvel endpoint utilise numpy, nous n'essayons pas d'installer les dépendances manquantes).